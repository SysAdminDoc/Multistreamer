...
test('rendered host, viewer, import, dialog, and mobile workflows', { timeout: 120000 }, async () => {
    ...
    await context.route('https://www.googleapis.com/youtube/v3/videos**', route => route.fulfill({
        status: 200,
        contentType: 'application/json',
        body: JSON.stringify({ items: [{ liveStreamingDetails: { concurrentViewers: '4321' } }] })
    }));
    ...
    assert.equal(persistedSnapshot.config.version, 16);
    ...
    await host.fill('#youtubeApiKey', 'stats-test-key');
    await host.click('#settingsPanel button:has-text("Save Key Locally")');
    await host.check('#statsOverlayToggle');
    await host.waitForFunction(() => settings.stats.enabled === true && streamStats.get('dQw4w9WgXcQ')?.count === 4321);
    await host.waitForFunction(() => {
        const badge = document.querySelector('[data-stats-badge="dQw4w9WgXcQ"]');
        return badge && !badge.hidden && badge.textContent.includes('4.3K');
    });
    assert.match(await host.textContent('[data-stats-badge="dQw4w9WgXcQ"]'), /4\.3K watching/);
    ...
    assert.deepEqual(exported.settings.stats, { enabled: true, refreshSeconds: 60 });
